...
async def delete_user_history(client: Client, message: Message):
    ...
    if(can_delete):      
        ...
        reply_string="Noob, kamu tidak bisa menghapus keberadaan mereka !"
...
async def deleteall(client: Client, message: Message):
    ...
    dante = await client.edit(message, "__Deleting all messages from this user.__")
    user = message.reply_to_message.from_user.id

    await client.delete_user_history(message.chat.id, user)
    await message.delete(dante, "__All messages from this user has been deleted.__")
...
async def unblockall(client: Client, message: Message):
    ...
    dante = await client.edit(message, f"__Unblocking all users in {chat_name}.__")
